...
namespace PaperTanksV2_Client
{
    class GameEngine
    {
        protected uint targetWidth = 3840;   // 4K width
        protected uint targetHeight = 2160;  // 4K height
        ...
        protected void init()
        {
            ...
            this.window = new RenderWindow(new VideoMode((uint)this.displayWidth, (uint)this.displayHeight), title);
            this.window.Closed += (sender, e) =>
            {
                window.Close();
                this.isRunning = false;
            };
            this.window.KeyPressed += (sender, e) => { }; // TODO: Update Input State
            this.window.KeyReleased += (sender, e) => { }; // TODO: Update Input State
            this.window.MouseMoved += (sender, e) =>
            {
                var scaledMousePos = this.ScaleMousePosition(new Vector2i(e.X, e.Y));
            };
        ...
        protected void render(SKCanvas canvas)
        {
            ...
            using (SKPaint paint = new SKPaint())
            {
                ...
                SKRect rect = new SKRect(this.mouse.ScaledMousePosition.X, this.mouse.ScaledMousePosition.Y, this.mouse.ScaledMousePosition.X + 64, this.mouse.ScaledMousePosition.Y + 64);
